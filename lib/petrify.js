...
exports.withFiles = function(dirname, pattern, fn, callback){
  ...
  function recursiveReadDir(dirstart, cb) {
    // Use lstat to resolve symlink if we are passed a symlink
    fs.lstat(dirstart, function(err, stat) {
        if(err) {
            return cb(err);
        }
        ...
        function isDir(abspath) {
            fs.stat(abspath, function(err, stat) {
                if(stat.isDirectory()) {
                    ...
                    recursiveReadDir(abspath, function(err, data) {
        ...
        if(stat.isDirectory()) {
            fs.readdir(dirstart, function (err, files) {
                total = files.length;
                for(var x=0, l=files.length; x<l; x++) {
                    isDir(path.join(dirstart, files[x]));
                }
            });
        } else {
            return cb(new Error("path: " + dirstart + " is not a directory"));
        }
    });
 }

    recursiveReadDir(dirname, function(err, dirs_and_files){
        if(err) return callback(err);

        var files = dirs_and_files.files;
        var lengthWithSlash = (/^.*\/$/.test(dirname)) ? dirname.length : dirname.length+1;
        ...
        files.forEach(function(filename){
          var relative_filename = filename.slice(lengthWithSlash);
